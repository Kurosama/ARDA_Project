...
public class ARDA_algorithm {
    ...
    public double[][] get_points_array(double x, double y){
    	...
    	int point_x,point_y,num=0;
    	for(int i=0;i<9;i++){
    		for(int j=0;j<9;j++){
				num++;
    			points_array[num][0]= x+((j-4)/100);
    			points_array[num][1]= y+((i-4)/100);
    		}
    	}
    	return points_array;
    }
    
    //Copia dentro una lista le coppie di valori listaTiple
    public double[][] get_points_array2(double[][] listaTriple){
		double[][] points_array=new double[81][2];
		int num=0;
    	for(int i=0;i<9;i++){
    		for(int j=0;j<9;j++){
    			points_array[num][0]= listaTriple[num][0];
    			points_array[num][1]= listaTriple[num][1];
    			num++;
    		}
    	}
    	return points_array;
    ...
    public double[] get_points_prob_dist_array(){
    	...
    	for(int i=0;i<9;i++){
    		for(int j=0;j<9;j++){
    			points_array[num]=binom_dist[i]*binom_dist[j];
    			num++;
    		}
    	}
    	return points_array;
    ...
    public double[] get_acc_value_in_point(double[] pos,double[][][] grid,int[] grid_info){
    	...
    	if(x_loc>=0 && x_loc<grid_info[0] && y_loc>=0 && y_loc<grid_info[1]){
    		...
    		System.out.println("----> Errore nei parametri della matrice");
    		pos[0]=0; pos[1]=0;
    		return pos;
    	}
    }
    ...
    /* 	Stima della prossima posizione dopo "after_time" secondi per un singolo oggetto
     *
     *	"actual_point"  [2]	: Posizione attuale dell'oggetto
     *	"prev_point"	[2]	: Posizione precedente dell'oggetto
     *	"acc_info"    [n][2]: Matrice contenente le informazioni sull'accelerazione data all'oggetto dalla zona di importanza
     *	"brake_info"	[2]	: Matrice ocntenete le informazioni sulla frenata (attrito) datta dalla zona di importanza
     ...
     */
     ...
     public double[][] single_object_prediction(double[] actual_point,double[] prev_point, double [][][] acc_info, double [][] brake_info, int[] grid_info, double after_time, double brake_max){
     	...
     	//Ricalcolo la posizione finche after_time volte (visto che T_campione ha valore 1)
	...
	public double[][] predict_next_for(double[] actual_point, double[] prev_point,double [][][] acc_info,double[][] brake_info,int[] grid_info, double after_time, double brake_max){
		...
		/*-Creare l'insieme di oggetti che descrivono la distribuzione
		 * di probabilità della posizione attuale e precedente dell'utente. 
		...
		for(int i=0;i<9;i++){
    		for(int j=0;j<9;j++){
    			element=points_array[num];
    			element_result=single_object_prediction(element,prev_point,acc_info,brake_info,grid_info,after_time,brake_max);
    			actual_prob_dist=points_prob_dist_array[num];
    			getted_results[num][0]=element_result[0][0];
    			getted_results[num][1]=element_result[0][1];
    			getted_results[num][2]=element_result[1][0];
    			getted_results[num][3]=element_result[1][1];
    			getted_results[num][4]=actual_prob_dist;
    			num++;
    		}
    	}
    ...
    public double[][] predict_next_for2(double[][] TriplaUltimaPrevisione, double[][] TriplaPenultimaPrevisione,double [][][] acc_info,double[][] brake_info,int[] grid_info, double after_time, double brake_max){
		...
		for(int i=0;i<9;i++){
    		for(int j=0;j<9;j++){
    			element=points_array[num];
    			prev_element=prev_points_array[num];
